...
const PintoChannelConfigSchema = z.preprocess((raw) => {
    ...
    const value = { ...raw };
    if (value.webhookSecret === undefined &&
...
const getRawPintoChannelConfig = (cfg) => cfg?.channels?.pinto ?? {};
const hasTopLevelPintoConfig = (cfg) => {
    const channelConfig = getRawPintoChannelConfig(cfg);
    return Boolean(channelConfig &&
...
const resolveDefaultPintoAccountId = (cfg) => {
    const channelConfig = getRawPintoChannelConfig(cfg);
...
const findPintoAccountByBotId = (cfg, botId) => {
...
const normalizeWebhookPath = (value) => {
    const trimmed = typeof value === "string" ? value.trim() : DEFAULT_PINTO_WEBHOOK_PATH;
    if (!trimmed) {
        return DEFAULT_PINTO_WEBHOOK_PATH;
    }
    return trimmed.startsWith("/") ? trimmed : `/${trimmed}`;
};
const normalizeObserverAgentIds = (value) => {
    if (!Array.isArray(value)) {
        return undefined;
    }
...
const normalizePintoImageUrl = (value) => {
    if (typeof value !== "string") {
        return undefined;
    }
    const trimmed = value.trim();
...
const PINTO_SENSITIVE_LOG_KEY_PATTERN = /(authorization|credential|password|secret|signature|token|webhook_secret|api[-_]?key)/i;
const PINTO_URL_LOG_KEY_PATTERN = /(href|src|uri|url)$/i;
const sanitizePintoLogString = (value, key) => {
...
const resolvePintoImageUrlFromValue = (value, source, depth = 0) => {
    if (depth > 5) {
        return undefined;
    }
    ...
    if (Array.isArray(value)) {
        for (const [index, item] of value.entries()) {
            const resolved = resolvePintoImageUrlFromValue(item, `${source}[${index}]`, depth + 1);
            if (resolved) {
                return resolved;
            }
        }
    ...
    for (const key of PINTO_NESTED_IMAGE_URL_KEYS) {
        const resolved = resolvePintoImageUrlFromValue(record[key], `${source}.${key}`, depth + 1);
        if (resolved) {
            return resolved;
        }
    }
...
const resolvePintoInboundImageUrl = (payload) => {
...
const sanitizePintoWebhookPayloadForLog = (value, depth = 0, key) => {
    if (depth > 5) {
        return "[max-depth]";
    }
    if (typeof value === "string") {
...
const logPintoWebhookPayload = (log, payload, imageUrl) => {
    const record = payload;
    const keys = Object.keys(record).sort();
    const isImageEvent = payload.message === "chat.sentImage" ||
    ...
    log?.debug?.(`[PintoPlugin] Webhook payload keys: ${keys.join(", ") || "(none)"}`);
    if (!isImageEvent) {
        return;
    }
    const sanitizedPayload = JSON.stringify(sanitizePintoWebhookPayloadForLog(payload));
    if (imageUrl) {
        log?.info?.(`[PintoPlugin] Inbound image URL resolved from ${imageUrl.source}. Payload: ${sanitizedPayload}`);
        return;
    }
    log?.warn?.(`[PintoPlugin] Inbound image payload has no supported media URL. Payload keys: ${keys.join(", ") || "(none)"}. Payload: ${sanitizedPayload}`);
};
const inferPintoImageMimeType = (imageUrl) => {
...
const buildPintoImageMediaContext = (imageUrlValue) => {
...
const stripPintoPrefix = (id) => id.replace(/^pinto:/, "");
const buildPintoApiError = async (res) => {
...
const resolvePintoReplyText = (payload) => {
...
const resolvePintoReplyMediaUrls = (payload) => {
...
export const pintoPlugin = {
    ...
    security: {
        collectWarnings: ({ account }) => {
            ...
            if (!account?.config?.botId?.trim()) {
                warnings.push("Pinto botId is not configured. Set channels.pinto.botId to the real Pinto bot id.");
            }
            if (!normalizeWebhookSecret(account?.config?.webhookSecret)) {
                warnings.push("Pinto webhookSecret is empty. Set channels.pinto.webhookSecret if you want webhook secret validation.");
            }
            if (webhookPath !== (account?.config?.webhookPath?.trim() || webhookPath)) {
                warnings.push(`Pinto webhookPath should start with '/'. Use ${webhookPath} as channels.pinto.webhookPath.`);
            }
            return warnings;
        },
    },
    setup: {
        ...
        applyAccountConfig: ({ cfg, accountId, input, }) => {
            ...
            const resolvedWebhookSecret = normalizeWebhookSecret(resolved.webhookSecret);
            const nextBotId = input.botId !== undefined
            ...
            const nextObserverAgentIds = input.observerAgentIds !== undefined
                ? normalizeObserverAgentIds(input.observerAgentIds)
                : normalizeObserverAgentIds(resolved.observerAgentIds);
            const nextWebhookPath = input.webhookPath !== undefined
            ...
            return applySetupAccountConfigPatch({
                ...
                patch: {
                    enabled: true,
                    apiUrl: input.apiUrl?.trim() || resolved.apiUrl || DEFAULT_PINTO_API_URL,
                    ...(nextBotId ? { botId: nextBotId } : {}),
                    ...(nextAgentId ? { agentId: nextAgentId } : {}),
                    ...(nextObserverAgentIds
                        ? { observerAgentIds: nextObserverAgentIds }
                        : {}),
                    webhookSecret: (inputWebhookSecret ? input.webhookSecret : undefined) ||
    ...
    config: {
        listAccountIds: (cfg) => listPintoAccountIds(cfg),
        defaultAccountId: (cfg) => resolveDefaultPintoAccountId(cfg),
        ...
        describeAccount: (account) => ({
            ...
            observerAgentIds: normalizeObserverAgentIds(account.config?.observerAgentIds) || [],
            webhookPath: account.config?.webhookPath?.trim() || DEFAULT_PINTO_WEBHOOK_PATH,
        }),
    },
    outbound: {
        deliveryMode: "direct",
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            const observerAgentIds = normalizeObserverAgentIds(account?.observerAgentIds)?.filter((agentId) => agentId !== configuredAgentId) || [];
            ...
            const unregister = registerPluginHttpRoute({
                ...
                handler: async (req, res) => {
                    try {
                        ...
                        const targetObserverAgentIds = normalizeObserverAgentIds(targetAccount?.observerAgentIds)?.filter((agentId) => agentId !== targetAgentId) || [];
                        const configuredSecret = normalizeWebhookSecret(targetAccount?.webhookSecret);
                        ...
                        const buildMsgCtx = (sessionKey, accountId) => ctx.channelRuntime.reply.finalizeInboundContext({
                        ...
                        for (const observerAgentId of targetObserverAgentIds) {
                            const observerSessionKey = ctx.channelRuntime.routing.buildAgentSessionKey({
                                agentId: observerAgentId,
                                channel: "pinto",
                                accountId: targetAccountId,
                                peer,
                            });
                            const observerCtx = buildMsgCtx(observerSessionKey, targetAccountId);
                            ...
                            void ctx.channelRuntime.reply
                                ...
                                .catch((error) => {
                                ctx.log?.warn?.(`[PintoPlugin] Observer agent ${observerAgentId} failed: ${error?.message ?? String(error)}`);
                            });
                        }
